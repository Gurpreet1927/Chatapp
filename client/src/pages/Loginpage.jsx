...
import assets from '../assets/assets';
import { AuthContext } from '../../context/AuthContext';
import { login, signup, formatMessageTime } from '../lib/utils.js'; // ✅ Fixed import
...
const Loginpage = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    setLoading(true);
    try {
      await authLogin(currState === "Sign up" ? 'signup' : 'login', {
        fullName,
        email,
        password,
        bio,
  ...
  return (
    <div className="min-h-screen bg-cover bg-center flex items-center justify-content gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl">
      ...
      >
        <h2 className="font-medium text-2xl flex justify-content items-center">
          {currState}
          {isDataSubmitted && (
            <img
        ...
        <button
          type="submit"
          disabled={loading}
          className={`py-3 bg-gradient-to-r from-blue-400 to-purple-600 text-white rounded-md cursor-pointer ${
