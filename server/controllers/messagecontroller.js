...
import User from "../models/user.js";
import Group from "../models/group.js";
import cloudinary from "../lib/cloudnary.js"; // Make sure this file is spelled correctly
import { io, userSocketMap } from "../server.js";

// ✅ Get users for sidebar + unseen message count
export const getUsersForSidebar = async (req, res) => {
  try {
    const userId = req.user._id;
    const users = await User.find({ _id: { $ne: userId } }).select("-password");
...
// ✅ Get all messages with a user or group
export const getMessages = async (req, res) => {
  try {
    const selectedId = req.params.id;
    const myId = req.user._id;
    ...
    if (group) {
      messages = await Message.find({ groupId: selectedId })
        .sort({ createdAt: 1 })
